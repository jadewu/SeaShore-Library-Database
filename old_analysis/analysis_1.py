...
def analysis_1():
    ...
    if request.method == "POST":
        ...
        plt.title('Top borrowed books daily')
        ...
        for i in range(1,13):
            sql = "select count(request_id),book_name,book_id from customer_request where request_status <> 'W' and months_between(sysdate,request_start) <= :1 and months_between(sysdate,request_start) >= :2 group by book_id, book_name order by 1 desc OFFSET 0 ROWS FETCH NEXT 3 ROWS ONLY"
            val = [i,i-1]
            cursor.execute(sql, val)
            data = cursor.fetchall()
            pre = -1
        ...
        plt.legend()
        plt.ylabel('Borrowed Numbers')
        plt.title('Top borrowed books monthly')
